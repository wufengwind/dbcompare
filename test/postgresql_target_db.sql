-- PostgreSQL Target Database Test Script
-- 创建与源数据库有差异的数据库结构用于测试比较

-- 创建数据库 (需要手动执行)
-- CREATE DATABASE ecommerce_target ENCODING 'UTF8';
...
DROP TABLE IF EXISTS target_only_table CASCADE;
...
SELECT 
    ...
    AVG(cr.rating::DECIMAL) AS avg_rating
FROM products p
...
RETURNS TABLE(user_id INTEGER, username VARCHAR, email VARCHAR, first_name VARCHAR, last_name VARCHAR, country VARCHAR)
LANGUAGE plpgsql
AS $$
BEGIN
    RETURN QUERY
    SELECT u.user_id, u.username, u.email, u.first_name, u.last_name, u.country
    FROM users u
...
CREATE OR REPLACE FUNCTION target_only_procedure(p_id INTEGER, p_value DECIMAL(10,2))
RETURNS VOID
LANGUAGE plpgsql
AS $$
BEGIN
...
CREATE OR REPLACE FUNCTION get_discounted_price(
    ...
    p_discount_rate DECIMAL(3,2) DEFAULT 0.15
)
RETURNS DECIMAL(10,2)
LANGUAGE plpgsql
AS $$
...
CREATE OR REPLACE FUNCTION target_only_function(p_input TEXT)
RETURNS TEXT
LANGUAGE plpgsql
AS $$
BEGIN
...
CREATE OR REPLACE FUNCTION review_validation_function()
RETURNS TRIGGER
LANGUAGE plpgsql
AS $$
BEGIN
...
CREATE OR REPLACE FUNCTION target_only_trigger_function()
RETURNS TRIGGER
LANGUAGE plpgsql
AS $$
BEGIN
...
CREATE TRIGGER review_validation
    ...
    FOR EACH ROW
    EXECUTE FUNCTION review_validation_function();

CREATE TRIGGER target_only_trigger
    BEFORE INSERT ON target_only_table
    FOR EACH ROW
    EXECUTE FUNCTION target_only_trigger_function();
...
-- 显示成功消息
SELECT 'PostgreSQL Target Database created successfully!' AS message; 