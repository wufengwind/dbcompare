...
REM 设置默认参数
set JAR_FILE=..\target\db-compare-tool-1.0.0.jar
set SOURCE_DB=jdbc:mysql://localhost:3306/source_db?useSSL=false^&serverTimezone=UTC
set TARGET_DB=jdbc:mysql://localhost:3306/target_db?useSSL=false^&serverTimezone=UTC
set DB_USER=root
...
REM 询问用户是否要修改连接参数
set /p change_config="是否要修改数据库连接参数? (y/N): "
if /i "%change_config%"=="y" (
    set /p DB_USER="请输入数据库用户名 [%DB_USER%]: "
    set /p DB_PASSWORD="请输入数据库密码 [%DB_PASSWORD%]: "
    set /p mysql_host="请输入MySQL主机地址 [localhost]: "
    if "!mysql_host!"=="" set mysql_host=localhost
    set /p mysql_port="请输入MySQL端口 [3306]: "
    if "!mysql_port!"=="" set mysql_port=3306
    
    set SOURCE_DB=jdbc:mysql://!mysql_host!:!mysql_port!/source_db?useSSL=false^&serverTimezone=UTC
    set TARGET_DB=jdbc:mysql://!mysql_host!:!mysql_port!/target_db?useSSL=false^&serverTimezone=UTC
)
...
java -jar "%JAR_FILE%" ^
  ...
  --output test_results\tables_views_comparison.txt ^
  --format TXT ^
  --verbose
...
java -jar "%JAR_FILE%" ^
  ...
  --output test_results\no_index_constraint_comparison.txt ^
  --format TXT ^
  --verbose
...
REM 检查是否要打开HTML报告
set /p open_html="是否要打开HTML报告? (y/N): "
if /i "%open_html%"=="y" (
    if exist "test_results\comparison_report.html" (
        start test_results\comparison_report.html
    ) else (
        echo HTML报告文件不存在
    )
)
