...
dmprogram :
    (sql_clauses SEMI)* sql_clauses? ('/' EOF? | EOF)
    ;
...
alter_diskgroup_stmt :
    ALTER DISKGROUP LT_STRING pathname CORRUPT
    ;
...
decl_variable :
    variable_name_list (AS | CONSTANT)? plsql_datatype not_null? default_clause_option? SEMI
    ;
...
plsql_datatype :
    datatype
    | qualified_name dblink_clause? '%' (TYPE | ROWTYPE)
    | REF qualified_name
    | CURSOR
    ;
...
flashback_tab_stmt :
    FLASHBACK TABLE full_table_name_list TO flashback_tab_element
    ;

flashback_tab_element :
    BEFORE DROP rename?
    | BEFORE TRUNCATE
    | TIMESTAMP exp flashback_trig_enable?
    | scn_or_lsn trxid flashback_trig_enable?
    ;
...
alter_system_set_stmt :
    ALTER SYSTEM SET LT_STRING '=' raw_exp defer? scope?
    ;
...
alter_session_stmt :
    ALTER SESSION sess_id? alter_session_option
    ;

alter_session_option :
    SET QUERY_REWRITE_INTEGRITY assign_op exp
    | SET EVENTS LT_STRING
    | SET TIME_ZONE assign_op set_time_zone_string
    | SET LT_STRING '=' raw_exp purge?
    | SET CASE_SENSITIVE assign_op (exp | DEFAULT)
    | SET CURRENT_SCHEMA '=' qualified_name
    | SET sess_attr '=' sess_attr_val
    | (ENABLE | DISABLE) PARALLEL parallel_mode
    | FORCE PARALLEL parallel_mode parallel_degree?
    ;
...
alter_proc_stmt :
    ALTER PROCEDURE full_proc_name COMPILE cascade_opt? DEBUG?
    ;

alter_func_stmt :
    ALTER FUNCTION full_fun_name COMPILE cascade_opt? DEBUG?
    ;

alter_package_stmt :
    ALTER pkg_type full_proc_name COMPILE declare_opt? cascade_opt? DEBUG?
    ;
...
alter_table_stmt :
    ALTER TABLE full_table_name alter_table_action
    ;

alter_index_stmt :
    ALTER INDEX full_index_name alter_index_action
    ;
...
alter_table_partition_action :
    ...
    | EXCHANGE (PARTITION | SUBPARTITION) full_partition_name WITH TABLE full_table_name including_indexes?
    ...
    | MODIFY (PARTITION | SUBPARTITION) full_partition_name ADD add_subpartition_desc
    | MODIFY (PARTITION | SUBPARTITION) full_partition_name (ADD | DROP) VALUES '(' list_partition_value_list ')'
    | RENAME (PARTITION | SUBPARTITION) full_partition_name TO full_partition_name
    | MOVE (PARTITION | SUBPARTITION) full_partition_name TABLESPACE tablespace_name
...
alter_table_action :
    ADD COLUMN? not_exist? '(' column_def_list ')'
    | ALTER COLUMN? column_name SET DEFAULT (ON NULL)? default_exp
    ...
    | ALTER COLUMN? column_name RENAME TO column_name
    ...
    | ADD COLUMN? not_exist? column_def
    | DROP COLUMN? exist? '(' drop_column_list ')'
    | DROP COLUMN? exist? column_name restrict_cascade?
    ...
    | DROP IDENTITY
    | DROP AUTO_INCREMENT
    | ADD COLUMN? not_exist? column_name identity_clause
    | (ENABLE | DISABLE) ata_lock_option
    | ENABLE CONSTRAINT constraint_name check_option_def_true?
    | DISABLE CONSTRAINT constraint_name restrict_cascade?
    | ALTER COLUMN? column_name SET not_null_spec
    | MODIFY (ERROR | LOG) TO ctab_error_option
    | ALTER COLUMN? '(' column_list ')' SET STAT NONE?
    | ALTER COLUMN? column_name SET STAT NONE
    | ALTER COLUMN? column_name SET NOT? VISIBLE
    | (WITH | WITHOUT) COUNTER
    | LOGGING
    | NOLOGGING
    | REBUILD COLUMNS
    | TRUNCATE (PARTITION | SUBPARTITION) truncate_partition_name reuse_storage_option?
    | TRUNCATE (PARTITION | SUBPARTITION) FOR '(' exp_list ')' reuse_storage_option?
    ...
    | (WITH | WITHOUT | TRUNCATE) ADVANCED LOG
    | (ADD | DROP) LOGIC LOG
    ...
    | increment_set
    | ALTER COLUMN? column_name (ADD | DROP) user_clause
...
column_def_4_option :
    ...
    | auto_update_clause
    | default_clause_with_on_null_opt identity_clause
    | default_clause_with_on_null_opt column_constraint_def
    | default_clause_with_on_null_opt auto_update_clause
...
refs_spec_action :
    foreign_key? REFERENCES PENDANT? refd_table_and_columns match_option? ref_triggered_action?
    ;
...
column_list_list :
    '(' column_list ')' as_alias? storage_tag? (COMMA '(' column_list ')' as_alias? storage_tag?)*
    ;
...
alter_trigger_stmt :
    ALTER TRIGGER full_trigger_name (alter_trigger_option | RENAME TO trigger_name)
    ;
...
alter_materialized_view_stmt :
    ALTER MATERIALIZED VIEW full_view_name alter_table_partition_action_options? mv_refresh_clause? query_rewrite?
    ;

alter_view_stmt :
    ALTER VIEW full_view_name alter_view_action
    ;
...
exec_proc_stmt :
    EXECUTE full_proc_name dblink_clause? (raw_exp_list | param_list)?
    ;
...
alter_tablespace_set_stmt :
    ALTER TABLESPACE SET id (ADD | DELETE) '(' id_list ')'
    ;
...
alter_tablespace_stmt :
    ALTER TABLESPACE tablespace_name alter_tablespace_action
    | ALTER HUGE TABLESPACE tablespace_name ADD PATH pathname
    ;
...
backup_stmt :
    BACKUP back_range_option with_bak_dir_list? backup_to_options? backup_path_null backup_desc_options? backup_maxsize?
    backup_limit? backup_identified? backup_compressed? backup_without? backup_trace_file_level? backup_tsk_thread_num_null backup_parallel_dir?
    ;
...
restore_stmt :
    ...
    | RESTORE TABLESPACE tablespace_name restore_datafile_lst? FROM restore_from restore_identified? with_bak_arch_dir_list? restore_file_list_options? mirror_file_list_options? restore_mapped_file? restore_trace_file_level? restore_tsk_thread_num_null restore_parallel? res_until?
    ;
...
res_without_index_constraint :
    (WITH | WITHOUT) INDEX ((WITH | WITHOUT) CONSTRAINT)?
...
stat_on_org_stmt :
    STAT stat_para stat_size? ON qualified_name '(' column_list ')' GLOBAL?
    | STAT stat_para stat_size? ON INDEX qualified_name GLOBAL?
    | STAT ON qualified_name GLOBAL?
    ;
...
create_index_stmt :
    CREATE replace_option? partition_tag? unique_tag? INDEX not_exist? index_name with_inner? ON full_table_name '(' index_column_list ')'
    bm_join_index_clause? GLOBAL? partition_clause? storage_tag? index_no_sort? online_options? reverse_options? unusable_options? parallel_stmt?
    ;
...
storage :
    (INITIAL | NEXT | MINEXTENTS | FILLFACTOR | BRANCH) lt_integer
    | ON PRIMARY
    | ON id
    | CACHE '=' id
...
id_list :
    (id | PRIMARY) (COMMA (id | PRIMARY))*
    ;

create_proc_stmt :
    CREATE replace_option? PROCEDURE not_exist? full_proc_name2 create_proc_option
    ;

create_proc_option :
    WRAPPED LT_STRING
    | (WITH ENCRYPTION)? param_def_list_option? invoker_rights_clause_options? func_action
    ;
...
package_def :
    ...
    | method_inherit_options? method_attr_options? member_static? FUNCTION id param_def_list_option? RETURN plsql_datatype
    func_call_option2_options? (pipelined_options | PIPELINED USING plsql_datatype)? SEMI
    ;
...
package_body_def :
    ...
    | method_inherit_options? method_attr_options? member_static? FUNCTION id param_def_list_option? RETURN plsql_datatype
    func_call_option2_options? pipelined_options? check_exec_options? is_as dmsubprogram SEMI
    ;
...
object_def :
    ...
    | method_inherit_options? method_attr_options? member_static? FUNCTION id param_def_list_option? RETURN plsql_datatype deterministic_clause_options? (pipelined_options | PIPELINED USING plsql_datatype)?
...
alter_crypto_stmt :
    ALTER CRYPTO crypto_name alter_crypto_action
    ;
...
alter_crypto_action :
    ...
    | (ADD | DROP) CRYPTO HASH LT_STRING
    ;

comment_stmt :
    COMMENT ON TABLE full_table_name IS LT_STRING
    | COMMENT ON COLUMN full_column_name IS LT_STRING
    | COMMENT ON VIEW full_view_name IS LT_STRING
    ;
...
create_table_action :
    full_table_name '(' table_element_list ')' create_table_action_list_option
...
create_table_action_list_option :
    ctab_append_attr_clause? compress_tag? rowdependencies_clause? ctab_row_movement_clause? ctab_log_options? advance_log_clause? add_log_clause? ctab_error_options? distribute_by? increment?
    | FROM DEFAULT DIRECTORY id LOCATION '(' pathname ')'
    | FROM DATAFILE? pathname parameters_option_list?
    | FROM DATAFILE DEFAULT DIRECTORY id LOCATION '(' pathname ')' parameters_option_list?
    | USING PARTITION GROUP full_table_name BY '(' column_list ')' pg_sub_partition? lock_partitions_clause? storage_tag? compress_tag? rowdependencies_clause? ctab_row_movement_clause? ctab_log_options? advance_log_clause? add_log_clause? ctab_error_options?
    ;
...
range_distribute_act_lst :
    range_distribute_act (COMMA range_distribute_act)*
    ;
...
at_raft :
    EXECUTE AT_KEYWORD (id | lt_integer)
    ;

create_trigger_stmt :
    CREATE replace_option? TRIGGER full_trigger_name2 (WITH ENCRYPTION)?  before_after
    (trig_del_ins_upd_list local? ON full_table_name2 | trig_event_list local? ON event_object at_raft?) trig_referencing_def_options? trig_for_each_option? trig_when_option? trig_enable? dmsubprogram
...
trig_referencing_old :
    OLD ROW? AS? trig_old_name
    ;

trig_referencing_new :
    NEW ROW? AS? trig_new_name
    ;
...
month_rate :
    FOR EACH lt_integer MONTH (day_in_month | day_in_month_week)
    ;
...
day_in_month_week :
    DAY lt_integer OF WEEK (lt_integer | LAST)
    ;
...
times_in_day :
    duaring_time? FOR EACH lt_integer (MINUTE | SECOND)
    ;
...
repeat_interval :
    frequency_clause interval_clause_option?
    ;
...
interval_clause_option :
    SEMI interval_clause_list
;
...
mv_log_purge_clause :
    PURGE REPEAT exp
    | PURGE START WITH exp ((REPEAT | NEXT) exp)?
...
create_materialized_view_log_stmt :
    CREATE MATERIALIZED VIEW LOG ON full_table_name horizon_partition_clause? space_limit? storage_tag? compress_tag? ctab_log_options? mv_log_with_clause? mv_log_purge_clause?
    ;
...
delete_stmt_body :
    FROM? from_tv where_current_option return_into_obj? limit_option? check_limit_option? log_errors?
    | FROM? full_tv_name delete_multi_tv_option where_current_option return_into_obj? limit_option? check_limit_option? log_errors?
    ;

delete_multi_tv_option :
    (FROM | USING) from_tv_list
    ;
...
alter_database_stmt :
    ALTER DATABASE alter_database_action
    | ALTER DATABASE ADD NODE LOGFILE file_list
    | ALTER DATABASE ADD NODE
    | ALTER SYSTEM alter_system_action
    ;
...
alter_database_action :
    ...
    | OPEN force?
    | SUSPEND
    | (NORMAL | PRIMARY | STANDBY) force?
    | (ADD | DELETE | MODIFY) ARCHIVELOG LT_STRING
    | NOARCHIVELOG
    | ARCHIVELOG CURRENT?
...
insert_stmt_body :
    INTO? (full_tv_name | select_with_paran) alias_option? insert_tail return_into_obj? log_errors?
...
rollback_stmt :
    ROLLBACK WORK? commit_tail? to_savepoint?
    ;
...
row_limiting_clause :
    FETCH first_next_desc (top_exp PERCENT?)? row_num_desc (ONLY | WITH TIES)
    | OFFSET top_exp row_num_desc (FETCH first_next_desc (top_exp PERCENT?)? row_num_desc (ONLY | WITH TIES))?
    ;
...
pivot_sfun :
    invocation_expression alias_option?
    ;
...
pivot_in_clause1_expr :
    exp alias_option?
    | '(' exp_list_2 ')' alias_option?
    | ANY
    ;

pivot_in_clause_low_1 :
    pivot_in_clause1_expr (COMMA pivot_in_clause1_expr)*
    ;
...
normal_tv :
    ...
    | ARRAY pexp_pfx range_from_to? alias_option?
    | from_table_exp range_from_to? alias_option?
    | from_table_select_with_paran range_from_to? alias_option?
    | XMLTABLE '(' exp xml_passing? xmlcoldef_lst_options? ')' alias_option?
    | JSON_TABLE '(' argument COMMA LT_STRING on_error? jsoncol_lst ')' alias_option?
    | normal_tv_derived_table
...
grouping_set_items :
    grouping_set_item (COMMA grouping_set_item)*
    ;
...
without_into_select :
    simple_select (sel_clause_app for_xml_path?)?
    | select_with_paran sel_clause_app for_xml_path?
    ;
...
simple_select :
    query_exp (((UNION | EXCEPT | MINUS | INTERSECT) all_distinct_option? corresponding_clause? select_clause)+)?
    | select_with_paran ( (UNION | EXCEPT | MINUS | INTERSECT) all_distinct_option? corresponding_clause? select_clause)+
    ;
...
set_value_node :
    full_column_name assign_op (DEFAULT | exp)
    | '(' set_col_list ')' assign_op (select_with_paran | record_var_value)
...
alter_profile_stmt :
    ALTER PROFILE profile_name resource_limit_options?
    ;

create_user_stmt :
    CREATE USER not_exist? user_name create_user_option?
    ;

create_user_option :
    IDENTIFIED authent_type pwd_policy? account_lock? user_encrypt_options? space_limit? read_only_flag? resource? expire? allow_ip_list? not_allow_ip_list? allow_dt_list? not_allow_dt_list? default_ts_name? quota_ts?
    ;
...
alter_user_stmt :
    ALTER USER user_name authent_type_options? replace_old_pwd? pwd_policy? account_lock? user_encrypt_options? space_limit2? read_only_flag? resource? expire? allow_ip_list? not_allow_ip_list? allow_dt_list? not_allow_dt_list? default_ts_name? on_schema? quota_ts?
    | ALTER USER user_name (GRANT | REVOKE)? CONNECT THROUGH user_name
    ;
...
resource_limit_list_with_comma :
    resource_limit (COMMA resource_limit)*
    ;
...
resource_limit :
    resource_limit_option resource_limit_value
    ;
...
create_dblink_stmt :
    CREATE replace_option? PUBLIC? LINK not_exist? qualified_name CONNECT db_type_str? WITH login_name IDENTIFIED BY (password | WRAPPED LT_STRING) USING LT_STRING dblink_option_lst_options?
    ;
...
create_domain_stmt :
    CREATE DOMAIN not_exist? qualified_name AS? datatype charset_option? domain_default_option? domain_constraints_option?
    ;
...
create_character_set_stmt :
    CREATE CHARACTER SET not_exist? character_set_name AS? character_set_source collate_clause_option?
    ;
...
create_sequence_stmt :
    ...
    | ALTER SEQUENCE sequence_name sequence_option_list_options?
    ;
...
grant_stmt :
    GRANT grant_stmt_body
    ;

grant_stmt_body :
    grant_privs ON (db_object | qualified_name)? full_object_name TO grantees with_grant_option? by_grantor?
    | (db_priv_list | grant_privs) TO grantees with_admin_option? by_grantor?
    ;
...
exp :
    ...
    | function_call
    | contains_exp
    | pexp_pfx exp_pfx_option
    | invocation_expression exp_invocation_option
...
exp_pfx_option :
     '%' (ROWCOUNT | ROWCOUNT2 | IS OPEN | FOUND | NOT FOUND)
     | '(' afun_arg_lst ')' OVER '(' afun_partition_by? order_by_options? afun_windowing? ')'
     ;

exp_invocation_option :
     OVER '(' afun_partition_by? order_by_options? afun_windowing? ')'
     |  from_first_last_option '(' afun_partition_by? order_by_options? afun_windowing? ')'
     ;

from_first_last_option :
    FROM (FIRST | LAST) ((IGNORE | RESPECT) NULLS)? OVER
    ;
...
in_value_exp :
    ...
    | decode_case
    | pexp_pfx '%' (ROWCOUNT | ROWCOUNT2)
    | global_var
    | NEXT VALUE FOR pexp_pfx
    | (ARRAYLEN | CHAR | REVERSE) '(' in_value_exp ')'
    | ROWNUM
    | sizeof_expression
...
pexp_pfx_option :
   '.' (id dblink_clause? | key | '*')  ('[' exp_list ']')*
   |'(' opt_argument_list? ')' ( keep_clause |  within_clause )?  ('[' exp_list ']')*
   |'.' ( EXECUTE | EXTRACT  )'(' opt_argument_list? ')' ('[' exp_list ']')*
   ;
...
pexp_c2_insert :
    ...
    | ':' trig_new_name '.' column_name
    | NEW '.' column_name
    | member2 '%' BULK (EXCEPTION | ROWCOUNT)
    ;
...
invocation_expression_low :
    ('.' EXECUTE '(' opt_argument_list? ')')+
    | ('(' opt_argument_list? ')' (keep_clause | within_clause)? )+
    ;
...
keep_clause :
    KEEP '(' DENSE_RANK (FIRST | LAST) order_by_options? ')'
    ;
...
contains_exp :
    ...
    | CONTAINS '(' exp COMMA contains_query_exp (COMMA contains_query_exp_lst)? ')'
    ;
...
bool_exp :
    exp bool_exp_compound_option?
    | '(' exp_list_2 ')' bool_exp_list_compound_option
...
bool_exp_compound_option :
   IS NOT? JSON strict_option? with_unique_option?
   | IS NOT? OF type_option? '(' type_element_list ')'
   | IS NOT? NULL
   | NOT? BETWEEN exp AND exp
   | NOT? LIKE exp (ESCAPE exp)?
   | NOT? MEMBER OF exp
   | NOT? IN (select_with_paran | in_value_list | '(' value_list ')')
   | ops exp
   | ops query_any_options (select_with_paran | '(' value_list ')')
;

bool_exp_list_compound_option :
   | NOT? IN (select_with_paran |  ('(' value_list_set ')'))
   | ops query_any_options? (select_with_paran | ('(' value_list_set ')'))
   | OVERLAPS '(' exp_list_2 ')'
   ;
...
raw_id :
    LT_IDENTIFIER
    | sql_reserved_word    // todo :  add this option considering some special cases, it actually expands the scope
    ;
...
array_wrapper :
    ...
    | WITH (UNCONDITIONAL | CONDITIONAL)? wrapper_flag
    ;
...
error_handle :
    (NULL | ERROR | EMPTY | TRUE | FALSE) ON ERROR
    ;
...
lt_integer :
    LT_INTEGER
    | LT_REAL   //todo :  add this option considering some special cases, it actually expands the scope
    ;
...
alter_contextindex_stmt :
    ALTER CONTEXT INDEX index_name ON full_table_name cti_sync_option online_options? lexer_clause?
    ;
...
opt_argument_list :
    ...
    | all_distinct_option ( exp | exp_list_2 )
    | '*'
    | '+'
    | without_into_select
    ;
...
json_fun_tail :
    returning? pretty? array_wrapper? json_tail_on_empty? json_tail_on_error?
    ;
...
cursor_option_2 :
    (IS | FOR) without_into_select2
    | (IS | FOR) joined_table
    | (IS | FOR) TABLE full_tv_name
