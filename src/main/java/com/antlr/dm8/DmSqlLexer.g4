...
CONNECT:                               'CONNECT';
CONNECT_BY_IS_CYCLE:                   'CONNECT_BY_ISCYCLE';
CONNECT_BY_IS_LEAF:                    'CONNECT_BY_ISLEAF';
...
// Skip
WHILE_SPACE:                            [ \t\r\n]+    -> skip;
WHILE_COMMENT_OPEN:                     '/*'          ->channel(HIDDEN),pushMode(WHILE_COMMENT);
LINE_COMMENT:                           '--' ~[\r\n]* -> channel(HIDDEN);
C_LINE_COMENT:                          '//' ~[\r\n]* -> channel(HIDDEN);
...
// Const
LT_STRING:                             'N'? '\'' (~'\'' | '\'\'' | '\'' WHITESPACE_WITH_NEWLINE '\'')* '\'';
LT_BITSTRING:                          [B] '\''  (~'\'' | '\'' WHITESPACE_WITH_NEWLINE '\'')*  '\'';
LT_INTEGER:                            (DIGIT+ | [X]'\''(~'\'' | '\'' WHITESPACE_WITH_NEWLINE '\'')+ '\'');
LT_BINTEGER:                           '0' [X] (DIGIT|[A-F])+;
LT_DECIMAL:                            (DIGIT* '.' DIGIT+) ;
LT_REAL:                               (DIGIT+ | LT_DECIMAL)('E'([+-]? DIGIT+)? ('D' | 'F')?);
...
fragment WHITESPACE_WITH_NEWLINE:      [ \t\f]* [\n\r] [ \t\n\r\f]*;

/* TODO : Qsign
fragment XQSIGN_START:                  [^0-9a-zA-Z \f\n\r\t];
fragment XQSIGN_END:                    [^0-9a-zA-Z \f\n\r\t];
fragment XQSIGN_INSIDE:                 [0-9a-zA-Z \f\n\r\t]+;

fragment LT_QSIGN_STRING:              'Q' '\'' XQSIGN_START XQSIGN_INSIDE XQSIGN_END '\'';
*/
...
mode WHILE_COMMENT;
XCSTART:                                '/*'        ->channel(HIDDEN);
XCSTOP:                                 '*/'        ->channel(HIDDEN),popMode;
XCINSIDE_sign:                          [*/]        ->channel(HIDDEN);
XCINSIDE:                               ~[*/]+      ->channel(HIDDEN);
