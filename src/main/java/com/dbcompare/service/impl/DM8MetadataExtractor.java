...
/**
 * DM8 database metadata extractor implementation
 */
public class DM8MetadataExtractor implements DatabaseMetadataExtractor {
    
    private static final Logger logger = LoggerFactory.getLogger(DM8MetadataExtractor.class);
    ...
    private List<DatabaseObject> extractTables(Connection connection, String schema) throws SQLException {
        ...
        String sql = "SELECT TABLE_NAME FROM USER_TABLES WHERE OWNER = ? OR OWNER = USER";
        ...
        try (PreparedStatement stmt = connection.prepareStatement(sql)) {
            ...
            // If USER_TABLES doesn't work, try DBA_TABLES or ALL_TABLES
            logger.warn("无法从USER_TABLES查询表，尝试使用ALL_TABLES: {}", e.getMessage());
    ...
    private List<DatabaseObject> extractViews(Connection connection, String schema) throws SQLException {
        ...
        String sql = "SELECT VIEW_NAME FROM USER_VIEWS WHERE OWNER = ? OR OWNER = USER";
        ...
        try (PreparedStatement stmt = connection.prepareStatement(sql)) {
            ...
            logger.warn("无法从USER_VIEWS查询视图，尝试使用ALL_VIEWS: {}", e.getMessage());
    ...
    private List<DatabaseObject> extractProcedures(Connection connection, String schema) throws SQLException {
        ...
        String sql = "SELECT OBJECT_NAME FROM USER_PROCEDURES WHERE OBJECT_TYPE = 'PROCEDURE' AND (OWNER = ? OR OWNER = USER)";
        ...
        try (PreparedStatement stmt = connection.prepareStatement(sql)) {
            ...
            logger.warn("无法从USER_PROCEDURES查询过程，尝试使用ALL_PROCEDURES: {}", e.getMessage());
            sql = "SELECT OBJECT_NAME FROM ALL_PROCEDURES WHERE OBJECT_TYPE = 'PROCEDURE' AND OWNER = ?";
            try (PreparedStatement stmt = connection.prepareStatement(sql)) {
                stmt.setString(1, schema.toUpperCase());
                try (ResultSet rs = stmt.executeQuery()) {
                    while (rs.next()) {
    ...
    private List<DatabaseObject> extractFunctions(Connection connection, String schema) throws SQLException {
        ...
        String sql = "SELECT OBJECT_NAME FROM USER_PROCEDURES WHERE OBJECT_TYPE = 'FUNCTION' AND (OWNER = ? OR OWNER = USER)";
        ...
        try (PreparedStatement stmt = connection.prepareStatement(sql)) {
            ...
            logger.warn("无法从USER_PROCEDURES查询函数，尝试使用ALL_PROCEDURES: {}", e.getMessage());
            sql = "SELECT OBJECT_NAME FROM ALL_PROCEDURES WHERE OBJECT_TYPE = 'FUNCTION' AND OWNER = ?";
            try (PreparedStatement stmt = connection.prepareStatement(sql)) {
                stmt.setString(1, schema.toUpperCase());
                try (ResultSet rs = stmt.executeQuery()) {
                    while (rs.next()) {
    ...
    private List<DatabaseObject> extractTriggers(Connection connection, String schema) throws SQLException {
        ...
        String sql = "SELECT TRIGGER_NAME FROM USER_TRIGGERS WHERE OWNER = ? OR OWNER = USER";
        ...
        try (PreparedStatement stmt = connection.prepareStatement(sql)) {
            ...
            logger.warn("无法从USER_TRIGGERS查询触发器，尝试使用ALL_TRIGGERS: {}", e.getMessage());
            sql = "SELECT TRIGGER_NAME FROM ALL_TRIGGERS WHERE OWNER = ?";
            try (PreparedStatement stmt = connection.prepareStatement(sql)) {
                stmt.setString(1, schema.toUpperCase());
                try (ResultSet rs = stmt.executeQuery()) {
                    while (rs.next()) {
    ...
    private List<DatabaseObject> extractIndexes(Connection connection, String schema) throws SQLException {
        ...
        String sql = "SELECT INDEX_NAME, TABLE_NAME FROM USER_INDEXES WHERE UNIQUENESS = 'NONUNIQUE' AND (OWNER = ? OR OWNER = USER)";
        ...
        try (PreparedStatement stmt = connection.prepareStatement(sql)) {
            ...
            logger.warn("无法从USER_INDEXES查询索引，尝试使用ALL_INDEXES: {}", e.getMessage());
            sql = "SELECT INDEX_NAME, TABLE_NAME FROM ALL_INDEXES WHERE UNIQUENESS = 'NONUNIQUE' AND OWNER = ?";
            try (PreparedStatement stmt = connection.prepareStatement(sql)) {
                stmt.setString(1, schema.toUpperCase());
                try (ResultSet rs = stmt.executeQuery()) {
                    while (rs.next()) {
    ...
    public String getDDL(DatabaseConfig config, DatabaseObject object) throws SQLException {
        try (Connection connection = getConnection(config)) {
            switch (object.getType()) {
                ...
                default:
                    return "-- DM8不支持 " + object.getType() + " 类型的DDL提取";
            }
        }
    }
    ...
    private String getIndexDDL(Connection connection, DatabaseObject index) throws SQLException {
        StringBuilder ddl = new StringBuilder();
        // Extract table name and index name from the combined name
    ...
    public boolean testConnection(DatabaseConfig config) {
        try (Connection connection = getConnection(config)) {
            ...
            logger.error("DM8数据库连接测试失败: {}", e.getMessage());
            return false;
        }
    }
